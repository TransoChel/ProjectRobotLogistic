...
int main(void)
{
    ...
    */
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        else rl_DrawText("[H] FLAG_WINDOW_HIDDEN: off", 10, 140, 10, MAROON);
        if (IsWindowState(FLAG_WINDOW_MINIMIZED)) rl_DrawText("[N] FLAG_WINDOW_MINIMIZED: on", 10, 160, 10, LIME);
        else rl_DrawText("[N] FLAG_WINDOW_MINIMIZED: off", 10, 160, 10, MAROON);
        ...
        rl_DrawText("Following flags can only be set before window creation:", 10, 300, 10, GRAY);
        if (IsWindowState(FLAG_WINDOW_HIGHDPI)) rl_DrawText("FLAG_WINDOW_HIGHDPI: on", 10, 320, 10, LIME);
        else rl_DrawText("FLAG_WINDOW_HIGHDPI: off", 10, 320, 10, MAROON);
        if (IsWindowState(FLAG_WINDOW_TRANSPARENT)) rl_DrawText("FLAG_WINDOW_TRANSPARENT: on", 10, 340, 10, LIME);
        else rl_DrawText("FLAG_WINDOW_TRANSPARENT: off", 10, 340, 10, MAROON);
        if (IsWindowState(FLAG_MSAA_4X_HINT)) rl_DrawText("FLAG_MSAA_4X_HINT: on", 10, 360, 10, LIME);
        else rl_DrawText("FLAG_MSAA_4X_HINT: off", 10, 360, 10, MAROON);
